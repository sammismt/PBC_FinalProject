...
class WebDriver:
    ...
    def __init__(self):
        Chrome_path = "C:\\Users\\marina.hsieh\\Desktop\\chromedriver_win32\\chromedriver.exe"
    ...
    if opentime_list[index][1] != "NA": 
        if opentime_list[index][1] == "close":
            ...
            if opentime_list[index][3] == opentime_list[index][4] == "NA":
                ...
                if int(opentime_list[index][1]) < time_var < int(opentime_list[index][2]):
                    return True
                else:
                    return False
            elif opentime_list[index][3] != "NA" and opentime_list[index][4] != "NA":
                # 中間有休息:星期幾，第一次營業時間，第二次營業時間，第一次打烊時間，第二次打烊時間
                if int(opentime_list[index][1]) < time_var < int(opentime_list[index][3]) or int(opentime_list[index][2]) < time_var < int(opentime_list[index][4]):
                    return True
                else:
                    return False
        
    elif opentime_list[index][1] == opentime_list[index][2] == opentime_list[index][3] == opentime_list[index][4] == "NA":
        return False
...
def keyword_index(list_keyword, find_keyword):
    ...
    for x in range(len(find_keyword)):
        ...
        for y in range(len(list_keyword)):
            if list_keyword[y] == str(find_keyword[x]):
        ...
        if points < 5:
            for y in range(len(goodwords_list)):
                if (goodwords_list[y] in str(comment_list[x])) == True:
                    points += (0.1) * str(comment_list[x]).count(goodwords_list[y])
        elif points == 5:
            break
...
def algorithm(final_result):
    ...
    for x in range(len(final_result)):
        rate_index = 0
        if open_or_not(final_result[x]["opentime"], day, time_var) == True and final_result[x]["name"] != "NA":
            
            if len(keywords) != 0:
                rate_index += keyword_index(final_result[x]["keyword"], keywords)
            elif len(keywords) == 0:
                rate_index += 12
...
def search_by_selenium():
    ...
    # range可以調整要記錄的店家數目
    final_result = []
    for r in range(min(30, len(url_list))):
    ...
    # 把挑選的店家的網頁開啟
    global new_driver
    new_driver = webdriver.Chrome(executable_path = "C:\\Users\\marina.hsieh\\Desktop\\chromedriver_win32\\chromedriver.exe")
    new_driver.maximize_window()
    try:
        for i in range(len(output_index_list)):
            if i == 0:
                new_driver.get(url_list[output_index_list[i]])  # 前往第x個地點的網址
                new_driver.implicitly_wait(20)

            else:
                # 如果是windows系統，command 要改成ctrl
                pyautogui.hotkey('ctrl', 't', interval=0.1)  # 開一個新分頁
                new_driver.switch_to.window(new_driver.window_handles[i])  # 前往新分頁
                new_driver.implicitly_wait(20)
...
search_Button = tk.Button(app, text = '查詢！', command = search_by_selenium, fg = '#8592A2', width=10)
