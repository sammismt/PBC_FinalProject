...
def algorithm(final_result):
    ...
    shop = []  # 爬蟲下來的大大list
    goodwords_list = ["好吃", "氣氛好", "服務好", "交通方便", "划算", "美味", "CP值高", "很不錯", "很讚", "喜歡", "很好"]  # 我們自己寫的正面辭庫
    ...
    for x in range(len(final_result)):
        ...
        if open_or_not(final_result[x]["opentime"], day, time_var) == True and final_result[x]["name"] != "NA":
            ...
            rate_index += goodwords_index(final_result[x]["review"], goodwords_list)
    ...
    output_list = [answer, output_index_list, rate_index_sorted, not_pass, restnum]
