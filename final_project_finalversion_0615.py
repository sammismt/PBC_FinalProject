...
class WebDriver:
    ...
    def __init__(self):
        ...
        self.location_data["opentime"] = [
            ["星期一", "NA", "NA", "NA", "NA"], ["星期二", "NA", "NA", "NA", "NA"],
            ["星期三", "NA", "NA", "NA", "NA"], ["星期四", "NA", "NA", "NA", "NA"],
            ["星期五", "NA", "NA", "NA", "NA"], ["星期六", "NA", "NA", "NA", "NA"],
            ["星期日", "NA", "NA", "NA", "NA"]
        ]
    ...
    def open_browser(self, single_url):
        ...
        s = Service(r"C:\\Users\\marina.hsieh\\Desktop\\chromedriver_win32\\ \
                    chromedriver.exe")
...
def open_or_not(opentime_list, day, time_var):
    ...
    index = 0
    for x in range(7):
        if day in opentime_list[index][0] is True:
            index == x
    ...
    for x in range(len(comment_list)):
        if points < 5:
            for y in range(len(goodwords_list)):
                if (goodwords_list[y] in str(comment_list[x])) is True:
                    points += (0.1) \
...
def algorithm(final_result):
    ...
    for x in range(len(final_result)):
        rate_index = 0  # 店家的分數
        if open_or_not(final_result[x]["opentime"], day, time_var) is True and
        ...
        elif open_or_not(final_result[x]["opentime"], day, time_var) is False:
...
def search_by_selenium():
    ...
    # 存取搜尋結果的所有店家連結
    s = Service(r"C:\\Users\\marina.hsieh\\Desktop\\\
    ...
    if not_pass != len(rate_index_sorted):
        try:
            global new_driver
            s = Service(r"""C:\\Users\\marina.hsieh\\
