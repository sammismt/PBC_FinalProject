...
def open_or_not(opentime_list, day, time_var):
    ...
    for x in range(7):
        if opentime_list[x][1] != "NA": 
            if opentime_list[x][0] == day and opentime_list[x][1] != "close":
                # 星期相同
                if opentime_list[x][3] == opentime_list[x][4] == "NA":
                    # 中午沒有休息:星期幾，營業時間，打烊時間，NA，NA
                    if int(opentime_list[x][1]) < time_var < int(opentime_list[x][2]):
                        return True
                    else:
                        return False
                elif opentime_list[x][3] != "NA" and opentime_list[x][4] != "NA":
                    # 中午有休息:星期幾，第一次營業時間，第二次營業時間，第一次打烊時間，第二次打烊時間
                    if int(opentime_list[x][1]) < time_var < int(opentime_list[x][3]) or int(opentime_list[x][2]) < time_var < int(opentime_list[x][4]):
                        return True
                    else:
                        return False
...
def algorithm(final_result):
    ...
    for x in range(len(final_result)):
        ...
        elif open_or_not(final_result[x]["opentime"], day, time_var) == True:
            continue
...
def search_by_selenium():
    ...
    # answer = algorithm(final_result)
    # output_index_list = algorithm(final_result)[1]

    for i in range(restnum - 1):
        answer_str += algorithm(final_result)[i]
        answer_str += '、'
    answer_str += algorithm(final_result)[restnum - 1]
    ...
    # try:
        # for i in range(len(output_index_list)):
            # if i == 0:
                # driver.get(url_list[output_index_list[i]])  # 前往第x個地點的網址
            ...
            # else:
                ...
                # driver.implicitly_wait(20)
                # driver.get(url_list[output_index_list[i]])
    # except:
        # pass
